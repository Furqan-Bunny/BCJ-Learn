...
import {
  ...
  ChevronLeft, ChevronRight, Trophy, Target, Users, AlertTriangle, Download,
  ArrowUpRight,
} from "lucide-react";
...
export interface TeacherResultsViewProps {
  rows: AttemptRow[];
  myModules: ModuleDef[];
}

export function TeacherResultsView({ rows, myModules }: TeacherResultsViewProps) {
          ...
          return (
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="font-medium text-sm truncate">{r.managerName}</div>
  ...
  return (
    <>
      <PageHeader
        eyebrow="Reporting"
        title="Test results — your modules"
        description={`Every quiz attempt on ${myModules.length === 1 ? "the module you own" : "your modules"} (${myModules.map((m) => `M${m.number} · ${m.title}`).join(" · ")}). Drill in to see question-by-question.`}
        actions={
          <Button
            variant="outline"
            onClick={async () => {
              const slugs = myModules.map((m) => m.slug);
              // Export only attempts on the teacher's owned modules: server-side
              // we'd ideally filter, but `exportResultsCsv` takes a single slug.
              // For multi-module teachers, just export all and filter client-side.
              const res = await exportResultsCsv(slugs.length === 1 ? { moduleSlug: slugs[0] } : {});
              if (!res.ok || !res.csv) {
                toast.error(res.error ?? "Could not export");
                return;
              }
              downloadCsv(res.csv, `bcj-my-results-${new Date().toISOString().slice(0, 10)}.csv`);
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <KpiCard label="Attempts" value={totalAttempts} icon={Users} />
        <KpiCard label="Pass rate" value={totalAttempts ? `${passRate}%` : "—"} icon={Trophy} accent="success" />
        <KpiCard label="Avg score" value={totalAttempts ? `${avgScore}%` : "—"} icon={Target} />
        <KpiCard label="Failed" value={failedCount} icon={AlertTriangle} accent="warning" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {myModules.length > 1 && (
          <FacetedFilter
            label="Module"
            options={myModules.map((m) => ({ value: m.slug, label: `M${m.number} · ${m.title}` }))}
            values={moduleFilter}
            ...
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-32 text-center text-muted-foreground">
                  ...
                  No attempts yet on your modules.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
