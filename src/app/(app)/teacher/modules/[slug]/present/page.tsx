...
import {
  ...
  X, Maximize2, Minimize2, ChevronUp, ChevronDown, BookOpen, GraduationCap, Sparkles,
} from "lucide-react";
import { moduleBySlug, moduleTotalMinutes } from "@/data/modules";
import { teachers } from "@/data/users";
import type { ContentType, LessonContent } from "@/types";
...
export default function PresenterPage(props: PageProps<"/teacher/modules/[slug]/present">) {
  const { slug } = use(props.params);
  const router = useRouter();
  const mod = moduleBySlug(slug);
  if (!mod) return notFound();

  // Flatten all content items in lesson order — single linear playlist
  ...
  // Track fullscreen state (handles ESC key exits + programmatic toggles)
  React.useEffect(() => {
    function onChange() {
  ...
  async function enterFullscreen() {
    ...
    try {
      ...
      // Some browsers reject the request if it doesn't follow a user gesture.
      // The toggle button still works because it's directly user-initiated.
    }
  }
  ...
  const startSessionStore = useAttendanceStore((s) => s.startSession);
  const endSessionStore = useAttendanceStore((s) => s.endSession);

  // Combined "Start session" action: starts the timer + flags session live + enters fullscreen
  function startSession() {
    setRunning(true);
    startSessionStore(slug);
    void enterFullscreen();
  }

  const current = playlist[idx];
  const teacher = teachers.find((t) => mod.ownerTeacherIds.includes(t.id));
  ...
  // Keyboard shortcuts: arrow keys, space, escape, F for fullscreen
  React.useEffect(() => {
  ...
  function next() {
    ...
    setRunning(true); // moving forward = session is live
  }
  function prev() {
  ...
  // Progress: which lesson are we in, how far through this lesson
...
// ─── YouTubePresenter ──────────────────────────────────────────────────
// Wraps a YouTube embed with:
//   • autoplay (driven by the seminar's "running" state)
//   • auto-advance to next content item when the video ends
//   • clean rel=0 (no related videos) and modestbranding=1 for a cleaner look
//
// Implementation: uses YouTube's iframe postMessage API (no extra script tag).
// On iframe load, we send {event:"listening"} and request onStateChange events.
// When state === 0 (ENDED), we call onEnded() which advances the playlist.
function YouTubePresenter({
  ...
  // Cache the latest onEnded so the listener always calls fresh state
  const onEndedRef = React.useRef(onEnded);
  React.useEffect(() => {
    onEndedRef.current = onEnded;
  }, [onEnded]);

  // Build the embed URL with autoplay + jsapi + clean look
  const src = React.useMemo(() => {
    ...
    // origin helps YT API verify the message channel
  ...
  React.useEffect(() => {
    function handleMessage(e: MessageEvent) {
      // Only trust messages from YouTube
      ...
      // YT sends: { event: "onStateChange", info: number }
      // info codes: -1 unstarted, 0 ended, 1 playing, 2 paused, 3 buffering, 5 cued
  ...
  // After iframe loads, subscribe to state-change events
  function handleLoad() {
  ...
  return (
    <div className="aspect-video bg-black">
      <iframe
        ref={iframeRef}
        // The key forces a fresh iframe (and fresh autoplay) when src changes.
        key={src}
        src={src}
