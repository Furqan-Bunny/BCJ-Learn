...
import { scoreDistribution } from "@/data/queries";
...
export default function TeacherModuleResults(props: PageProps<"/teacher/modules/[slug]/results">) {
  ...
  const distribution = scoreDistribution(slug);

  // Most-missed questions
  const allQuestions = questionsForModule(slug);
  const mostMissed = [...allQuestions].sort((a, b) => b.missRate - a.missRate).slice(0, 5);
  ...
  return (
    <>
                ...
                <li key={q.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground mt-1 flex items-center gap-3">
                      ...
                      <span>Miss rate: <span className="font-mono font-semibold text-rose-600">{Math.round(q.missRate * 100)}%</span></span>
                    </div>
                  </div>
                </li>
              ))}
      ...
      {/* Roster — full attendance + quiz funnel */}
