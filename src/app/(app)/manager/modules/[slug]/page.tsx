...
import {
  ArrowRight, Calendar, Target, Clock, Layers, PlayCircle, FileText, Link2, Users,
...
import { teachers, managers } from "@/data/users";
...
export default function ManagerModulePage(props: PageProps<"/manager/modules/[slug]">) {
  ...
  // Demo: logged in as manager m-1 (Luke). Real app would come from auth.
  const me = managers[0];

  // Optional pre-study viewer (per scope §5.1.3) — clicking any content item opens it
  ...
  return (
    <>
      ...
      {/* Quiz status — replaces the old "Start quiz" button */}
      <div className="mb-6">
        <QuizStatusCard managerId={me.id} mod={mod} variant="full" />
      </div>

      {/* Stat strip */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <StatCard icon={Calendar} label="Training day" value={fmtDate(mod.scheduledDate)} />
      ...
      {/* How this module works — context for managers */}
          ...
          <Card key={lesson.id} className="overflow-hidden">
            <div className="grid grid-cols-[auto_1fr] gap-0">
              ...
              <div className="p-5">
                ...
                {/* Content items in this lesson — optional pre-study (per scope §5.1.3) */}
      ...
      {/* Optional self-study (flashcards) */}
      ...
      {/* Optional pre-study viewer — opens when manager taps a content item */}
