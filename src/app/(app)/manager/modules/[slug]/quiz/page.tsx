...
import { ArrowRight, ArrowLeft, Check, X, AlertCircle, Sparkles, Target, Clock, Layers } from "lucide-react";
...
import { ConfettiBurst, CountUp, motion, AnimatePresence } from "@/components/shared/animations";

type Phase = "intro" | "in-progress" | "submitted";

export default function QuizPage(props: PageProps<"/manager/modules/[slug]/quiz">) {
  const router = useRouter();
  const { slug } = use(props.params);
  const mod = moduleBySlug(slug);
  if (!mod) return notFound();

  // Demo: logged in as Luke (m-1). Real app would come from auth.
  const me = managers[0];

  // Determine which question pool to use:
  // - If the manager has a prior FAILED first-attempt in the current delivery,
  //   serve them the EASIER retake pool (per scope §4.1.3).
  // - Otherwise use first-attempt.
  ...
  const moduleResetTs = deliveryStartMap[slug] ? new Date(deliveryStartMap[slug]).getTime() : null;
  const personalResetIso = managerResetAtMap[`${slug}:${me.id}`];
  const personalTs = personalResetIso ? new Date(personalResetIso).getTime() : null;
  const cutoff = moduleResetTs && personalTs ? Math.max(moduleResetTs, personalTs) : moduleResetTs ?? personalTs;

  const myCurrentAttempts = allAttempts.filter((a) => {
    if (a.managerId !== me.id || a.moduleSlug !== slug) return false;
    if (cutoff && new Date(a.startedAt).getTime() < cutoff) return false;
    return true;
  });
  const hasFailedFirstAttempt = myCurrentAttempts.some(
    (a) => a.pool === "first-attempt" && a.status === "failed",
  );
  const activePool: QuestionPool = hasFailedFirstAttempt ? "retake" : "first-attempt";

  const allQuestions = React.useMemo(
    () => questionsForModule(slug, activePool).filter((q) => q.status === "approved").slice(0, mod.questionCount),
    [slug, mod.questionCount, activePool],
  );
  ...
  function submit() {
    ...
    setResult({ score, correct, total: allQuestions.length, passed });
    setPhase("submitted");
    const isRetake = activePool === "retake";
    toast(
      passed
        ? `🎉 Passed at ${score}%${isRetake ? " (retake)" : ""}`
        : isRetake
          ? `${score}% on retake — your trainer will reach out`
          : `${score}% — retake auto-scheduled`,
      {
        description: passed
          ...
          <Card>
            <CardContent className="p-6 md:p-8">
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                <Stat icon={Layers} label="Questions" value={`${allQuestions.length}`} />
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-violet-800/80 dark:text-violet-300/80 mt-0.5">
                        You're taking the easier retake pool. The questions cover the same material but are worded more directly.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6 flex items-center gap-3">
                <Button onClick={start} size="lg" className="h-12">
        ...
        <div className="max-w-3xl mx-auto pt-6">
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            {idx < allQuestions.length - 1 ? (
              ...
              <Button onClick={submit} disabled={Object.keys(answers).length < allQuestions.length / 2}>
                Submit quiz
              </Button>
            )}
          </div>
