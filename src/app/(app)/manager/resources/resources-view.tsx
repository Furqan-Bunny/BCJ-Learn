    ...
    <>
      ...
      <Dialog open={!!viewing} onOpenChange={(v) => { if (!v) setViewing(null); }}>
        <DialogContent className="sm:max-w-2xl">
          ...
          {(viewing?.storagePath || viewing?.externalUrl) ? (
            <div className="rounded-lg border bg-muted/30 p-6 text-sm text-muted-foreground text-center">
              <FileText className="size-8 mx-auto opacity-40 mb-2" />
              Document preview would render here.
              {viewing?.externalUrl && (
                <div className="mt-2">
                  <a href={viewing.externalUrl} target="_blank" rel="noopener noreferrer" className="text-primary underline">
                    Open in new tab
                  </a>
                </div>
              )}
