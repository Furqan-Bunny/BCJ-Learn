...
import { ArrowRight, BookOpen, Calendar, CheckCircle2, Clock, Sparkles, Target, Trophy } from "lucide-react";
...
export default function ManagerDashboard() {
  // For demo, show as if logged in as manager m-1
  const me = managers[0];
  ...
  const daysToNext = differenceInCalendarDays(new Date(nextModule.scheduledDate), new Date());

  const overallPct = Math.round((completedCount / orderedModules.length) * 100);
  const myActivity = activity.filter((a) => a.actorId === me.id || a.targetId === me.id).slice(0, 5);
  ...
  return (
    <>
      ...
      <div className="mb-6">
        <CheckInCard manager={{ id: me.id, name: me.name }} mod={nextModule} alwaysShow />
      </div>

      {/* Quiz status — replaces the old "Start quiz" / "Jump to quiz" buttons */}
      <div className="grid lg:grid-cols-[1fr_240px] gap-4">
        <QuizStatusCard managerId={me.id} mod={nextModule} variant="full" />

        {/* Progress ring stays visible alongside the status */}
      ...
      {/* Quick link to module materials when not in quiz-ready state */}
      ...
      <section className="mt-10">
        ...
        <Stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {orderedModules.map((m) => {
            ...
            const isNext = m.slug === nextModule.slug;
            const locked = !passed && !isNext && m.number > nextModule.number;
            const myAttempt = myAttempts.find((a) => a.moduleSlug === m.slug && a.status === "passed");
            return (
              <StaggerItem key={m.slug}>
              <Link href={`/manager/modules/${m.slug}`}>
                <Card className={`card-lift ${locked ? "opacity-60" : ""}`}>
      ...
      <section className="mt-10 grid lg:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Stat icon={Trophy} label="Modules passed" value={`${completedCount} of ${orderedModules.length}`} />
