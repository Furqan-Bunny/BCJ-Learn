...
export default function AdminManagers() {
  const [data] = React.useState<Manager[]>(managers);
          ...
          return (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => toast.success(`Reminder sent to ${m.name}`)}>
                  <Bell className="mr-2 size-4" /> Send reminder
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => toast(`Retake scheduled for ${m.name}`)}>
                  <RefreshCcw className="mr-2 size-4" /> Reassign retake
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="text-rose-600" onClick={() => toast(`${m.name} deactivated`)}>
