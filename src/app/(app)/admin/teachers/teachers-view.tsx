...
export interface AdminTeachersViewProps {
  teachers: Teacher[];
  modules: ModuleDef[];
  approvedByModule: Record<string, number>;
...
export function AdminTeachersView({ teachers, modules, approvedByModule, totalByModule }: AdminTeachersViewProps) {
  ...
  const enriched = teachers.map((t) => {
    ...
    const totalQs = ownedMods.reduce((s, m) => s + (totalByModule[m.slug] ?? 0), 0);
    const approvedQs = ownedMods.reduce((s, m) => s + (approvedByModule[m.slug] ?? 0), 0);
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div className="h-1.5 w-16 rounded-full bg-muted overflow-hidden">
                      <div className="h-full bg-emerald-500" style={{ width: totalQs ? `${(approvedQs / totalQs) * 100}%` : "0%" }} />
                    </div>
                  </div>
                </TableCell>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={async () => {
                          ...
                          const res = await deactivateUser(t.id);
              ...
              {ownedMods.length > 0 && (
                <div className="mt-4 pt-4 border-t">
                  ...
                  <div className="space-y-2">
                    {ownedMods.map((m) => {
                      const moduleApproved = approvedByModule[m.slug] ?? 0;
                      const moduleTotal = totalByModule[m.slug] ?? 0;
                      return (
