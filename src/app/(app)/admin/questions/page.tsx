...
export default function AdminQuestionLibrary() {
  ...
  // Per-module counts (always computed from full questions list)
  const countsByModule = React.useMemo(() => {
    const m: Record<string, { total: number; approved: number; pending: number }> = { [ALL]: { total: 0, approved: 0, pending: 0 } };
    for (const mod of modules) {
      m[mod.slug] = { total: 0, approved: 0, pending: 0 };
    }
  ...
  return (
    <>
      ...
      {/* Module selector — primary filter */}
      <div className="mb-6">
        ...
        <Card className="mb-6 bg-primary/[0.03] border-primary/30">
          <CardContent className="p-5">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                    <span>Owner: <span className="text-foreground font-medium">{activeModule.ownerTeacherIds.map((id) => teachers.find((t) => t.id === id)?.name).filter(Boolean).join(", ") || "—"}</span></span>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/admin/modules/${activeModule.slug}`}>
                    Module results
                  </Link>
                </Button>
              </div>
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              <MiniStat
                label="Total"
                value={countsByModule[activeModule.slug]?.total ?? 0}
                icon={ListChecks}
              />
              <MiniStat
                label="Approved"
                value={countsByModule[activeModule.slug]?.approved ?? 0}
                icon={CheckCircle2}
                tone="success"
              />
              <MiniStat
                label="Pending review"
                value={countsByModule[activeModule.slug]?.pending ?? 0}
                icon={Sparkles}
                tone="warn"
              />
            </div>
      ...
      {/* Secondary filters — search, pool, status */}
