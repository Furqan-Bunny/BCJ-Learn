...
const adminMessages: { kind: ActivityKind; tmpl: (name: string) => string }[] = [
  { kind: "user_added", tmpl: (n) => `${n} added a new Account Manager.` },
  { kind: "module_published", tmpl: (n) => `${n} published Module 2 — Quality Control & Safety.` },
  { kind: "reminder_sent", tmpl: (n) => `${n} sent a reminder to 12 overdue managers.` },
  ...
  { kind: "manager_flagged", tmpl: () => `System flagged a manager: failed retake on Module 1.` },
...
const NOTIF_KINDS: { kind: NotificationKind; subject: string; preview: string }[] = [
  ...
  { kind: "result", subject: "Retake scheduled — Module 1", preview: "Your manager will follow up with the retake date." },
